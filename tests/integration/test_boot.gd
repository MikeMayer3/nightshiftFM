...
func run(context: TestContext, tree: SceneTree) -> bool:
	...
	context.check(not boot.combat.mixer_open and boot.combat.manual_pause and boot.combat.session.paused, "Android Back closes mixer opened from pause without resuming")
	if boot.combat.mixer_open: boot.combat.close_mixer()
	boot.combat._open_settings()
	tree.root.go_back_requested.emit()
	context.check(boot.combat.settings_panel == null and boot.combat.session.paused, "Android Back closes in-run settings and preserves pause")
	boot.combat.toggle_pause()
	boot.combat.open_mixer()
	tree.root.go_back_requested.emit()
	context.check(not boot.combat.mixer_open and not boot.combat.manual_pause, "Android Back closes mixer opened from combat")
	boot.combat.session.hit_station(10000, &"M2_SWARMER_NAME")
	boot.combat._refresh()
	boot.combat.report_button.pressed.emit()
	context.check(boot.combat.report_open, "results contribution report opens")
	tree.root.go_back_requested.emit()
	context.check(not boot.combat.report_open and boot.combat.overlay.visible, "Android Back returns from contribution report to results")
