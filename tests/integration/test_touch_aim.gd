...
func run(context: TestContext, tree: SceneTree) -> bool:
	...
	context.check(screen.session.focus_active and screen.session.focus_point.is_equal_approx(world) and screen.session.active_combat.uses == 0, "second finger cannot steal aim or release the first finger")
	...
	context.check(screen.session.active_combat.uses == 0 and not screen.session.focus_active, "native release clears focus without firing a manual attack")
	touch(tree, target, true, 0)
	touch(tree, target, false, 0)
	context.check(screen.session.active_combat.uses == 0, "repeated tapping cannot trigger a manual attack")
	...
	context.check(not screen.session.focus_active and screen.session.active_combat.uses == 0, "Android canceled touch clears aim without firing")
	touch(tree, target, true, 0)
	drag(tree, Vector2(-20, -20), 0)
	touch(tree, Vector2(-20, -20), false, 0)
	context.check(not screen.session.focus_active and screen.session.active_combat.uses == 0, "release outside battlefield cancels native aim")
	touch(tree, target, true, 0)
	screen.toggle_pause()
	touch(tree, target, false, 0)
	screen.toggle_pause()
	drag(tree, target, 0)
	touch(tree, target, false, 0)
	context.check(not screen.session.focus_active and screen.session.active_combat.uses == 0, "pause clears touch ownership and stale release cannot fire")
	var overlay: Control = Control.new()
	overlay.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	screen.add_child(overlay)
	touch(tree, target, true, 0)
	touch(tree, target, false, 0)
	context.check(not screen.session.focus_active and screen.session.active_combat.uses == 0, "overlay intercepts new touches before arena acquisition")
	overlay.queue_free()
	await tree.process_frame
	mouse(tree, target, true)
	mouse(tree, target, false)
	context.check(screen.session.active_combat.uses == 0 and not screen.session.focus_active, "physical mouse still aims and releases through viewport")
	context.check(not screen.ability_button.visible, "obsolete cooldown control cannot receive native gestures")
	screen.queue_free()
	await tree.process_frame
	return true
