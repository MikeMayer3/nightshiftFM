	...
	for style: String in ["arc_aerial", "bass_driver", "static_net"]:
		for seed_value: int in [11, 42, 91]:
			...
			for mission: int in range(1, 4):
				...
				var saved: bool = MissionStore.new("user://m8_playthrough.json").save(profile, s.to_checkpoint()) == OK
				valid = valid and saved and s.is_finished()
				var row: Dictionary = {"mission": mission, "style": style, "seed": seed_value, "victory": won, "wave": s.wave, "seconds": s.elapsed, "hull": s.hull, "kills": s.kills, "breaches": s.breaches, "choices": decisions.size(), "restores": restored, "saved": saved, "cause": String(s.last_cause), "profile_cleared": profile.campaign.cleared}
				print(JSON.stringify(row))
				row["decisions"] = decisions
				rows.append(row)
				if not won: break
	var victories: int = rows.filter(func(row: Dictionary) -> bool: return row.victory).size()
	var passed: bool = valid and rows.size() == 27 and victories == 27
	FileAccess.open("res://docs/evidence/M8/playthrough.json", FileAccess.WRITE).store_string(JSON.stringify({"passed": passed, "valid": valid, "method": "Headless fixed-step combat, legal offers, earned unlocks; restore and continue each decision checkpoint. Not human playtesting.", "runs": rows}, "\t", true, true))
