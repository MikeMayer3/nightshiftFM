...
func capture(name: String) -> void:
	...
	root.get_texture().get_image().save_png("res://docs/evidence/M4-turrets/"+name+".png")
func _run() -> void:
	...
	screen.store = MissionStore.new("user://turrets_visual_fixture.json")
	root.add_child(screen)
	await process_frame
	screen.set_process(false)
	screen.session.start_active(42, &"run.1")
	# This presentation fixture equips all supports directly; it is not a valid
	# earned-upgrade save. Keep its artificial loadout out of the atomic store.
	...
	screen.session.supports.timers["arc_aerial"] = 0.5
	screen.session.supports.timers["bass_driver"] = 3.0
