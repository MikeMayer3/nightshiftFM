...
func run() -> void:
	...
	for step: int in 140000:
		...
		if s.is_wiring():
			...
			if not copy.restore_checkpoint(JSON.parse_string(JSON.stringify(s.to_checkpoint(), "", true, true))):
				FileAccess.open("res://docs/evidence/release-audit/simulation/" + "failed-checkpoint-%s-%d.json" % [mode, mission], FileAccess.WRITE).store_string(JSON.stringify(s.to_checkpoint(), "\t", true, true))
	...
	FileAccess.open("res://docs/evidence/release-audit/simulation/" + "playthrough-%s-%d-%s-%s-%s-%d.json" % [mode, mission, style, contract, main, difficulty], FileAccess.WRITE).store_string(JSON.stringify(row, "\t"))
