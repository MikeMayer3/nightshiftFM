class_name SignalDraft
extends DraftState
## Offensive choices and acquisitions share one earned pick; no shield quota or wave schedule.
const OVERDRIVE: StringName = &"consumable.overdrive"

static func is_new(id: StringName) -> bool:
	return String(id).begins_with("recruit.")

func card(id: StringName) -> UpgradeDefinition:
	if is_new(id) or id == OVERDRIVE:
		var option: UpgradeDefinition = UpgradeDefinition.new()
		option.id = id
		option.target_id = StringName(String(id).trim_prefix("recruit.")) if is_new(id) else &"main"
		var key: String = "SIGNAL_" + String(option.target_id).to_upper() + "_NEW" if is_new(id) else "SIGNAL_OVERDRIVE"
		option.name_key = StringName(key + "_NAME")
		option.description_key = StringName(key + "_DESC")
		option.required_rank = -1
		return option
...
func pool() -> Array[StringName]:
	...
	for definition: TrackDefinition in catalog:
		if definition.id == &"shield": continue
...
func _purchase(id: StringName) -> bool:
	...
	if is_new(id):
		...
		return family in [&"bass_driver", &"static_net"] and equip(family)
	var option: UpgradeDefinition = card(id)
	return option != null and option.target_id != &"shield" and track(option.target_id) != null and track(option.target_id).accept(id, [])
...
func restore(data: Variant) -> bool:
	...
	for id: StringName in [&"main", &"shield", &"arc_aerial"]: equip(id)
	...
	for id: StringName in banished:
		...
		if option == null or option.required_rank != 0 or option.target_id == &"shield": return false
