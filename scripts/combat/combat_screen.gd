...
func _refresh() -> void:
	...
	if session.is_finished():
		...
		details.text = tr("M3_RESULTS" if m3_enabled else "COMBAT_RESULTS") % [session.wave, session.elapsed, session.kills, session.intercepted, session.breaches, session.hull, cause]
	else:
...
func _save_checkpoint() -> void:
	...
	if session.phase == CombatSession.Phase.VICTORY: profile.commit_reward(session.run_id)
